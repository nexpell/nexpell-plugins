...
function getanznewscomments($id, $type)
{
    return mysqli_num_rows(
        safe_query(
            "SELECT commentID FROM `plugins_news_comments` WHERE `parentID` = " . (int)$id . " AND type='$type'"
        )
    );
}
...
function getanznewsrecomments($id, $type)
{
    return mysqli_num_rows(
        safe_query(
            "SELECT * FROM `plugins_news_comments_recomment` WHERE `parentID` = " . (int)$id . " AND type='$type'"
        )
    );
}
    ...
    $ds =
        safe_query(
            "SELECT
                `user_ID`
            FROM
                `plugins_news_comments_recomment`
            WHERE
                `parentID` = " . (int)$id . " AND
                `type` = '$type'
            ORDER BY
...
function getnewsrubricpic($rubricID)
{
    $ds =
        safe_query(
            "SELECT pic FROM `plugins_news_rubrics` WHERE `rubricID` = " . (int)$rubricID
        );
    if (mysqli_num_rows($ds) > '0') {
        $ds = mysqli_fetch_array($ds);
        return $ds['pic'];
