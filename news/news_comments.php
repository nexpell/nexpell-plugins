...
global $userID;
    ...
    $allowed = 0;
    $modul = $moduls[$type];
    $get = safe_query("SELECT ".$modul[2]." FROM $modul[0] WHERE ".$modul[1]."='".$parentID."'");
...
#-----------------------
	...
	if(isset($_GET['recom'])) { $_recom = $_GET['recom']; } else { $_recom=""; }
	if($_recom=="rem" and isset($_GET['rid'])) { 
		if(isanyadmin($userID) and is_numeric($_GET['rid'])) {
			try {
				safe_query("DELETE FROM `plugins_news_comments_recomment` WHERE `recoID` = '".$_GET['rid']."' LIMIT 1;");
				echo 'Done! <script>window.history.back();</script>';
				} Catch(Exception $e){
					// err
				echo '<script>window.history.back();</script>';
				}
		} else {
			echo '<b><font color="red">Access denied!</font></b><a href="javascript: window.history.back();">go back</a></script>';	
		}
		echo '<script>window.history.back();</script>';
		return false;
	}
	
	if(isset($_POST['saverecomment'])){
		if(!$userID) { echo "Access Denied!"; echo '<script>window.history.back();</script>'; return false; }
				try {
					safe_query("INSERT INTO `plugins_news_comments_recomment` (`comment_id`, `user_id`, `datetime`, `comment`, `type`, `parentID` ) VALUES ('".$_POST['comID']."', '".$_POST['userID']."', '".time()."', '".$_POST['message2']."', '".$_POST['type']."', '".$_POST['parentID']."');");
				} Catch(Exception $e) {
					//errs
				}
		echo '<script>window.history.back();</script>';
...
if (isset($_POST[ 'savevisitorcomment' ])) {
   ...
   if (mysqli_num_rows(safe_query("SELECT * FROM users WHERE username = '$name' "))) {
            $name = '*' . $name . '*';
        }
        $name = $name;
    }
   if (!empty($name) && !empty($message)) {
        $date = time();
        $ip = $GLOBALS[ 'ip' ];
        $ergebnis = safe_query("SELECT * FROM plugins_news_comments ORDER BY date DESC LIMIT 0,1");
        ...
        safe_query(
                "INSERT INTO `plugins_news_comments` (
                    `parentID`,
                    `type`,
                    `nickname`,
                    `date`,
...
//$dt = safe_query("SELECT userID FROM user WHERE pmoncomment='1'");

$dt = safe_query("SELECT userID FROM user_groups WHERE page='1'");
...
$name = getnickname($userID, $id);
    ...
    $msgfrom= array();
    $msgfrom[ 'ne' ] = ''.$plugin_language[ 'to_an_news' ].'';
...
//ende /////////////////////////////////////////////////////////////////////     
    ...
    foreach ($_POST[ 'commentID' ] as $id) {
        safe_query("DELETE FROM plugins_news_comments WHERE commentID='" . (int)$id."'");
    }
    ...
    if ($gesamt) {
        ...
        while ($ds = mysqli_fetch_array($ergebnis)) {
            ...
            if ($ds[ 'userID' ]) {
                ...
                $quotemessage = addslashes(getinput($ds[ 'newscomments' ]));
                ...
                if ($gethomepage != "" && $gethomepage != "http://" && $gethomepage != "http:///"
                    ...
                    $hp = '<a href="http://' . $gethomepage .
                        '" target="_blank"><i class="bi bi-house" title="' .
            ...
            $newscomments = $ds[ 'newscomments' ];
            
            if (isfeedbackadmin($userID) || isnewscommentposter($userID, $ds[ 'commentID' ])) {
                $edit = '<a class="badge text-bg-warning" href="index.php?site=news_manager&action=editcomment&id=' . $ds[ 'commentID' ] . '&amp;ref=' .
            ...
            $pm = new plugin_manager();
			$pm_pluginID = $pm->pluginID_by_name("recomment");
				if($pm_pluginID!=0) {
					$plugin_data = $pm->plugin_data("",$pm_pluginID);
					include($plugin_data['path']."news_recomments.php");
				}
					
            include_once("news_recomments.php"); 
            $cid=$ds['commentID']; 
            $recomment=""; 
            $recomm="";
            $recomment = zntdev_recomments_get($ds['commentID'], $userID);
           
            if($userID) { $recomm = '<a class="badge text-bg-success btn-sm" title="' .$plugin_language['comment_reply']. '" href="javascript: void(0)" onclick="javascript: document.getElementById(\'rec'.$cid.'\').style.display=\'inline\'"><span> ' .$plugin_language['comment_reply']. '</span></a>';
            } else { $recomm=""; }
			


            $data_array = array();
            $data_array['$avatar'] ='<img class="img-fluid avatar_small" src="./images/avatars/' . getavatar($ds[ 'userID' ]) . '">';
            $data_array['$newscomments'] = $newscomments;
            $data_array['$edit'] = $edit;
            $data_array['$actions'] = $actions;
            $data_array['$poster'] = $poster;
            $data_array['$recomm'] = $recomm;
            $data_array['$recomment'] = $recomment;
            $data_array['$cid'] = $ds[ 'commentID' ];
            $data_array['$date'] = $date;
            $data_array['$userID'] = $userID;
            $data_array['$referer'] = $referer;
            $data_array['$parentID'] = $parentID;

            $data_array['$your_comment_reply']=$plugin_language['your_comment_reply'];
            $data_array['$post_recomment']=$plugin_language['post_recomment'];
            $data_array['$comment']=$plugin_language['comment'];
